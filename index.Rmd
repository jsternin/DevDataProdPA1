...
## 1. Path from east to west between Top 10 US Cities  
...
Load data (coordinates, populatio, path)
...
addCircleMarkers(popup=sprintf(
  "Population: %d, Latitd: %5.2f, Lontitd: %5.2f %s",
   top10cities$pop,top10cities$Coord.lat,top10cities$Coord.lon,east_west$dist2west),
    label=top10cities$name, radius = sqrt(top10cities$pop/2000),clusterOptions=markerClusterOptions()) %>%  addPolylines(lat = east_west$lat, lng = east_west$lon)
